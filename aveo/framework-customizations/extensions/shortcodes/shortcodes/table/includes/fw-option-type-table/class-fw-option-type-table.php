...
class FW_Option_Type_Table extends FW_Option_Type
{
	...
	protected function _render( $id, $option, $data ) {
		...
		if ( ! $table_shortcode ) {
			trigger_error(
				__( 'table-builder option type must be inside the table shortcode', 'aveo' ),
				E_USER_ERROR
			);
		}
	...
	protected function _get_defaults() {
		...
		return apply_filters( 'fw_option_type_table_defaults', array(
			'header_options'  => array(
				'table_purpose' => array(
					'type'    => 'select',
					'label'   => esc_html__( 'Table Styling', 'aveo' ),
					'desc'    => esc_html__( 'Choose the table styling options', 'aveo' ),
					'choices' => array(
						'pricing' => esc_html__( 'Use the table as a pricing table', 'aveo' ),
						'tabular' => esc_html__( 'Use the table to display tabular data', 'aveo' ),
					),
					'value'   => 'pricing',
					'attr'    => array(
			...
			'row_options'     => array(
				'name' => array(
					...
					'desc'    => false,
					'choices' => array(
						'default-row' => esc_html__( 'Default row', 'aveo' ),
						'heading-row' => esc_html__( 'Heading row', 'aveo' ),
						'pricing-row' => esc_html__( 'Pricing row', 'aveo' ),
						'button-row'  => esc_html__( 'Button row', 'aveo' ),
						'switch-row'  => esc_html__( 'Row switch', 'aveo' )
					),
				)
			),
			'columns_options' => array(
				'name' => array(
					...
					'desc'    => false,
					'choices' => array(
						'default-col'   => esc_html__( 'Default column', 'aveo' ),
						'desc-col'      => esc_html__( 'Description column', 'aveo' ),
						'highlight-col' => esc_html__( 'Highlight column', 'aveo' ),
						'center-col'    => esc_html__( 'Center text column', 'aveo' )
					),
				)
			),
			'content_options' => array(
				...
				'pricing-row' => array(
					...
					'description' => array(
						...
						'value'        => '',
						'attr'         => array(
							'placeholder' => esc_html__( 'per month', 'aveo' )
						),
						'wrapper_attr' => array(
							'class' => 'fw-col-sm-6'
						)
				...
				'button-row'  => array(
					'button' => ( $button = $table->get_button_shortcode() )
						? array(
							'type'          => 'popup',
							'popup-title'   => esc_html__( 'Button', 'aveo' ),
							'button'        => esc_html__( 'Add', 'aveo' ),
