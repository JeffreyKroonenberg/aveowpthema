...
class FW_Shortcode_Map extends FW_Shortcode {
		...
		if (empty($this->data)) {
			$this->data = apply_filters('fw_shortcode_map_provider', array(
				'custom' => array(
					...
					'label'      => esc_html__('Custom', 'aveo'),
					'options'    => array(
						'locations' => array(
							'label' => esc_html__('Locations', 'aveo'),
							'popup-title' => esc_html__('Add/Edit Location', 'aveo'),
							'type' => 'addable-popup',
							'desc' => false,
							'template' => '{{  if (location.location !== "") {  print(location.location)} else { print("' . esc_html__('Note: Please set location', 'aveo') . '")} }}',
							'popup-options' => array(
								'location' => array(
									'type' => 'map',
									'label' =>esc_html__('Location', 'aveo'),
								),
								'title' => array(
									'type' => 'text',
									'label' => esc_html__('Location Title', 'aveo'),
									'desc' => esc_html__('Set location title', 'aveo'),
								),
								'description' => array(
									'type'  => 'textarea',
									'label' => esc_html__('Location Description', 'aveo'),
									'desc'  => esc_html__('Set location description', 'aveo')
								),
								'url' => array(
									'type'  => 'text',
									'label' => esc_html__('Location Url', 'aveo'),
									'desc'  => esc_html__('Set page url (Ex: http://example.com)', 'aveo'),
								),
								'thumb' => array(
									'label'       => esc_html__('Location Image', 'aveo'),
									'desc'        => esc_html__('Add location image', 'aveo'),
									'type'        => 'upload',
								)
							)
						)
	...
	protected function _render($atts, $content = null, $tag = '')
	{
		if (!isset($atts['data_provider']['population_method'])) {
			trigger_error(
				__('No location provider specified for map shortcode', 'aveo')
			);
			return '<b>' . esc_html__( 'Map Placeholder', 'aveo' ) . '</b>';
		}
		...
		if (!isset($this->data[$provider])) {
			return '<!-- WARNING: '
			       . sprintf(__('Unknown location provider "%s" specified for map shortcode', 'aveo'), $provider)
			       . ' -->';
		}
